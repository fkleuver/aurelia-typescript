...
describe("NavigationPlanStep", () => {
  let step: any;
  let state: any;
  let redirectInstruction: any;
  let firstInstruction: any;
  let sameAsFirstInstruction: any;
  let secondInstruction: any;
  let router;
  ...
  it("cancels on redirect configs", done => {
    step.run(redirectInstruction, state.next).then((e: any) => {
  ...
  describe("generates navigation plans", () => {
    it("with no prev step", done => {
      step.run(firstInstruction, state.next).then(() => {
    ...
    it("with prev step", done => {
      step.run(secondInstruction, state.next).then(() => {
        expect(state.result).toBe(true);
  ...
  describe("activation strategy", () => {
    ...
    it("is no-change when nothing changes", done => {
      ...
      step.run(sameAsFirstInstruction, state.next).then(() => {
        expect(state.result).toBe(true);
