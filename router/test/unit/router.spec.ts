...
describe("the router", () => {
  ...
  beforeEach(() => {
    ...
    router = new AppRouter(new Container(), history, new PipelineProvider(new Container()), undefined as any);
  });
  ...
  describe("generate", () => {
    it("should generate route URIs", done => {
      ...
      Promise.all([
        router.configure((config: any) => config.map({ name: "parent", route: "parent", moduleId: "./test" })),
        child.configure((config: any) => config.map({ name: "child", route: "child", moduleId: "./test" }))
      ]).then(() => {
    ...
    it("should delegate to parent when not configured", done => {
      ...
      router.configure((config: any) => config.map({ name: "test", route: "test/:id", moduleId: "./test" })).then(() => {
    ...
    it("should delegate to parent when generating unknown route", done => {
      ...
      Promise.all([
        router.configure((config: any) => config.map({ name: "parent", route: "parent/:id", moduleId: "./test" })),
        child.configure((config: any) => config.map({ name: "child", route: "child/:id", moduleId: "./test" }))
      ]).then(() => {
    ...
    it("should return a fully-qualified URL when options.absolute is true", done => {
      ...
      child.configure((config: any) => config.map({ name: "test", route: "test/:id", moduleId: "./test" })).then(() => {
  ...
  describe("_createNavigationInstruction", () => {
    it("should reject when router not configured", done => {
      router
        ...
        .then((x: any) => expect(true).toBeFalsy("should have rejected"))
        .catch((reason : any) => expect(reason).toBeTruthy())
        .then(done);
    });

    it("should reject when route not found", done => {
      router
        .configure((config: any) => config.map({ name: "test", route: "test/:id", moduleId: "./test" }))
    ...
    it("should resolve matching routes", done => {
      router
        ...
        .then((x: any) => expect(x).toEqual(jasmine.objectContaining({ fragment: "test/123", queryString: "foo=456" })))
    ...
    it("should reject when route is case sensitive", done => {
      router
        .configure((config: any) => config.map({ name: "test", route: "Test/:id", moduleId: "./test", caseSensitive: true }))
    ...
    describe("catchAllHandler", () => {
      let expectedInstructionShape = jasmine.objectContaining({
        config: jasmine.objectContaining({ moduleId: "test" })
      });

      it("should use a parent routers catchAllHandler if one exists", done => {
        const child = router.createChild(new Container());
        child.baseUrl = "empty";
        let expectedFirstInstructionShape = jasmine.objectContaining({
          config: jasmine.objectContaining({ moduleId: "./empty" })
        });
        let expectedSecondInstructionShape = jasmine.objectContaining({
          config: jasmine.objectContaining({ moduleId: "test" })
        });
        Promise.all([
          router.configure((config: any) => {
          ...
          child.configure((config: any) => {
            config.map([{ name: "empty", route: "", moduleId: "./child-empty" }]);
          })
        ])
          .then(() => {
            return router._createNavigationInstruction("foo/bar/123?bar=456").then((x: any) => {
              expect(x).toEqual(expectedFirstInstructionShape);
              return child
                ._createNavigationInstruction("bar/123?bar=456", x)
                .then((x1: any) => expect(x1).toEqual(expectedInstructionShape));
            });
          })
          .catch((reason : any) => fail(reason))
          .then(done);
      ...
      it("should use string moduleId handler", done => {
        router
          ...
          .then((x: any) => expect(x).toEqual(expectedInstructionShape))
          .catch((reason : any) => fail(reason))
          .then(done);
      });
      ...
      it("should use function handler", done => {
        router
          .configure((config: any) => {
            config.unknownRouteConfig = (instruction: any) => ({ moduleId: "test" });
          })
          .then(() => router._createNavigationInstruction("foo/123?bar=456"))
          .then((x: any) => expect(x).toEqual(expectedInstructionShape))
          .catch((reason : any) => fail(reason))
      ...
      it("should use async function handler", done => {
        router
          .configure((config: any) => {
            config.unknownRouteConfig = (instruction: any) => Promise.resolve({ moduleId: "test" });
          })
          .then(() => router._createNavigationInstruction("foo/123?bar=456"))
          .then((x: any) => expect(x).toEqual(expectedInstructionShape))
          .catch((reason : any) => fail(reason))
      ...
      it("should pass instruction to function handler", done => {
        router
          .configure((config: any) => {
            config.unknownRouteConfig = (instruction: any) => {
              expect(instruction).toEqual(
                jasmine.objectContaining({ fragment: "foo/123", queryString: "bar=456", config: null })
              );
      ...
      it("should reject invalid configs", done => {
        router
          .configure((config: any) => {
            config.unknownRouteConfig = (instruction: any) => null;
          })
          .then(() => router._createNavigationInstruction("foo/123?bar=456"))
          .then((x: any) => expect(true).toBeFalsy("should have rejected"))
          .catch((reason : any) => done());
      });
    });
  });
  ...
  describe("configure", () => {
    it("notifies when configured", done => {
      ...
      router.configure((config: any) => config.map({ route: "", moduleId: "./test" }));
    });

    it("notifies when already configured", done => {
      expect(router.isConfigured).toBe(false);

      router.configure((config: any) => config.map({ route: "", moduleId: "./test" })).then(() => {
        expect(router.isConfigured).toBe(true);
      ...
      router
        .configure((config: any) =>
          config.map([
            ...
            { name: "static", route: "static", moduleId: "static", href: staticHref, nav: true }
          ])
        )
        .then(() => {
